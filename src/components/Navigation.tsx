  ...
  return (
    <motion.header
      ...
      className={cn(
        ...
        isScrolled
          ? 'bg-dark-950/90 backdrop-blur-md border-b border-dark-800 shadow-lg'
          : 'bg-transparent'
      )}
      role="banner"
    >
      <nav className="container mx-auto px-4 sm:px-6 lg:px-8" aria-label="Navegação principal">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className={cn('hidden lg:flex items-center gap-8', isMobile ? 'absolute top-full left-0 right-0 bg-dark-950/95 backdrop-blur-md border-b border-dark-800 py-6 px-6 flex-col gap-4' : '')}>
            {navItems.map((item) => (
              <motion.button
                key={item.id}
        ...
        <AnimatePresence>
          {isMobileMenuOpen && isMobile && (
            <motion.div
              ...
              className="absolute top-full left-0 right-0 bg-dark-950/95 backdrop-blur-md border-b border-dark-800 py-6 px-6 flex flex-col gap-4"
            >
              {navItems.map((item) => (
                <motion.button
                  key={item.id}

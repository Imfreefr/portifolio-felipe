...
const whatsappOptions = [
  { key: 'orcamento' as const, label: 'Solicitar orçamento', icon: '💰' },
  { key: 'desenvolvimento' as const, label: 'Desenvolvimento web', icon: '💻' },
  { key: 'contato' as const, label: 'Conversar', icon: '💬' },
] as const;
...
export function Contact() {
  ...
  const whatsappDelays = useMemo(() => 
    whatsappOptions.map((_, i) => 0.4 + i * 0.1 + Math.random() * 0.2)
  , []);
      ...
      >
        ...
        >
          <div className="bg-preto-900/60 backdrop-blur-sm border border-preto-700 rounded-2xl p-8 lg:p-10">
            ...
            <p className="text-vinho-400 font-medium mb-8">
              {contactInfo.title}
            </p>

            <div className="space-y-6">
              <div className="flex items-start gap-4 p-4 bg-preto-950/50 rounded-xl border border-preto-700 group">
                ...
                <div>
                  <p className="text-neutral-500 text-sm uppercase tracking-wider font-medium mb-1">
                    WhatsApp
                  </p>
                  <a
              ...
              <div className="flex items-start gap-4 p-4 bg-preto-950/50 rounded-xl border border-preto-700">
                <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-vinho-500/10 flex items-center justify-center">
                  <MapPin className="w-6 h-6 text-vinho-400" aria-hidden="true" />
                </div>
                <div>
                  <p className="text-neutral-500 text-sm uppercase tracking-wider font-medium mb-1">
                    Localização
                  </p>
                  <p className="text-neutral-300">Brasil — Atendimento remoto</p>
                </div>
              </div>

              <div className="flex items-start gap-4 p-4 bg-preto-950/50 rounded-xl border border-preto-700">
                <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-vinho-500/10 flex items-center justify-center">
                  <Clock className="w-6 h-6 text-vinho-400" aria-hidden="true" />
                </div>
                <div>
                  <p className="text-neutral-500 text-sm uppercase tracking-wider font-medium mb-1">
                    Disponibilidade
                  </p>
                  <p className="text-neutral-300">Segunda a Sexta, 9h às 18h (BRT)</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="relative bg-preto-900/80 backdrop-blur-sm border border-preto-700 rounded-3xl p-8 h-full">
              <div className="h-full flex flex-col justify-center">
                ...
                >
                  ...
                  <p className="text-neutral-400">Clique no botão abaixo para abrir o WhatsApp com uma mensagem pré-definida.</p>
                </motion.div>
                ...
                <div className="space-y-4">
                  {whatsappOptions.map((item, index) => (
                    <motion.button
                      key={item.key}
                      onClick={() => handleWhatsAppClick(item.key)}
                      initial={{ opacity: 0, x: 20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ duration: reducedMotion ? 0 : 0.4, delay: whatsappDelays[index] }}
                      whileHover={{ x: reducedMotion ? 0 : 4 }}
                      whileTap={{ scale: reducedMotion ? 1 : 0.98 }}
                      className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-xl bg-preto-950/50 border border-preto-700 hover:border-vinho-500/30 hover:bg-preto-900 transition-all text-left group"
                    >
                      <span className="text-2xl" aria-hidden="true">{item.icon}</span>
                      <span className="font-medium text-white group-hover:text-vinho-400 transition-colors">{item.label}</span>
                      <ArrowRight className="w-5 h-5 text-neutral-500 group-hover:text-vinho-500 transition-colors" aria-hidden="true" />
                    </motion.button>
                  ))}
                </div>
                ...
                <motion.p
                  ...
                  className="mt-8 pt-6 border-t border-preto-700/50 text-center text-neutral-500 text-sm"
                >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-vinho-500 hover:text-vinho-400 font-medium inline-flex items-center gap-1 mt-2"
                  >
